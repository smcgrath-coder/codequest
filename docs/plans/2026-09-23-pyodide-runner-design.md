...
- **Errors covered:**
  ...
  - output cap
  - Python itself breaking (kind `Internal`, e.g. runaway recursion after `sys.setrecursionlimit(100000)`): the worker is restarted, as after an internal grading error, and the kid is told so, with only Python's last line under "What Python said". What the run printed is hidden, because Pyodide's own fatal-error handler prints a stack of the harness's frames there.
...
- **What it gives.** The room clears through the normal victory screen ("Marked done — half XP"), for half XP and never No Peeking. A boss still gives its badge. Chapters open on XP alone, so the half left out can mean a side quest or two before the next chapter. The character sheet lists marked-done rooms under "Marked done", so the kid can come back to them.
