...
export const BATCH_B = {
  ...
  // so it does when the 3 contacts are made with dict(), whatever it is given: dict(Amy="555-1111", ...),
  // dict(zip(names, phones)) or a list of pairs. Each dict() call but a copy is worked out again, as in
  // G13_DICT_CALL, and its contacts are known too (wrong/r4_dict_call_print_before_delete,
  // dict_zip_print_before_delete, dict_pairs_print_before_delete, dict_zip_header_print_before_delete).
  grind_13: {
    output: [{ expr: py`len(globals_of(dict)) >= 1 and (lambda d: (lambda A: (lambda C: (len(C) >= len(d) and (lambda P: all(len(p) == 1 and p[0] in d.items() for p in P) and sorted(repr(p[0]) for p in P) == sorted(map(repr, d.items())))([[kv for kv in A if str(kv[0]) in l and str(kv[1]) in l] for l in C[len(C) - len(d):]])) or (bool(C) and sorted(repr(kv) for kv in A if str(kv[0]) in C[-1] and str(kv[1]) in C[-1]) == sorted(map(repr, d.items()))))([l for l in L if any(str(k) in l and str(v) in l for k, v in A)]))(list(d.items()) + [kv for n in ast.walk(TREE) if isinstance(n, ast.Dict) and len(n.keys) == 3 and all(isinstance(x, ast.Constant) for x in n.keys + n.values) for kv in zip([x.value for x in n.keys], [x.value for x in n.values]) if kv not in d.items()] + [kv for n in ast.walk(TREE) if isinstance(n, ast.Assign) and isinstance(n.value, ast.Constant) for t in n.targets if isinstance(t, ast.Subscript) and isinstance(t.slice, ast.Constant) for kv in [(t.slice.value, n.value.value)] if kv not in d.items()] + [kv for n in ast.walk(TREE) if isinstance(n, ast.Call) and getattr(n.func, 'id', '') == 'dict' and not (len(n.args) == 1 and isinstance(n.args[0], ast.Name)) for kv in (lambda v: list(v.items()) if isinstance(v, dict) else [])(val(ast.unparse(n))) if kv not in d.items()]))(globals_of(dict)[0])`, hint: "Once you've added one contact and deleted one, print each contact that's left with its phone, using .items()." }],
    concepts: [
